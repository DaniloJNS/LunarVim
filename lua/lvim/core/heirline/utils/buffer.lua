...
function M.move(n)
  ...
  require("lvim.core.heirline.utils").event "BufsUpdated"
...
function M.close(bufnr, force)
  ...
  if require("lvim.core.heirline.utils").is_available "bufdelete.nvim" then
...
---@param compare_func string|function a string of a comparator defined in require("lvim.core.heirline.utils.buffer").comparator or a custom comparator function
...
function M.sort(compare_func, skip_autocmd)
  ...
  if type(compare_func) == "function" then
    ...
    vim.t.bufs = bufs
    if not skip_autocmd then require("lvim.core.heirline.utils").event "BufsUpdated" end
...
local function unique_path(bufnr)
  return require("lvim.core.heirline.utils.status").provider.unique_path() { bufnr = bufnr }
